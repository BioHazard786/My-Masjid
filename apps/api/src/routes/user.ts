...
import { pinMasjidSchema } from "@packages/validators";
...
const userRoutes = new Hono<{ Bindings: Bindings; Variables: Variables }>()
	...
	.get("/masjid/:id", async (c) => {
		...
		try {
			...
			if (!masjid) {
				return c.json(
					{ success: false, error: { message: "Masjid not found." } },
					404,
				);
			}
	...
	.get(
		...
		async (c) => {
			...
			const results = await db
				...
				.where(
					or(
						like(schema.masjids.nameEn, `%${q}%`),
						like(schema.masjids.nameHi, `%${q}%`),
						like(schema.masjids.nameUr, `%${q}%`),
					),
				)
				.limit(20);
	...
	.post("/masjids/pin", zValidator("json", pinMasjidSchema), async (c) => {
		const db = c.get("db");
		const { deviceId, masjidId, pushToken, preferredLanguage } =
			c.req.valid("json");
	...
	.post(
		...
		zValidator(
			"json",
			pinMasjidSchema.omit({ pushToken: true, preferredLanguage: true }),
		),
		async (c) => {
			const db = c.get("db");
			...
			try {
				await db
					...
					.where(
						and(
							eq(schema.userPinnedMasjids.deviceId, deviceId),
							eq(schema.userPinnedMasjids.masjidId, masjidId),
						),
					);
	...
	.post(
		...
		zValidator(
			"json",
			pinMasjidSchema.omit({ masjidId: true, pushToken: true }),
		),
		async (c) => {
			const db = c.get("db");
