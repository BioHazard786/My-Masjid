...
import { prayerTimesSchema } from "@packages/validators";
...
/**
 * Send push notifications directly to Expo's API
 */
 ...
 */
async function sendPrayerTimeNotifications(
	db: DrizzleD1Database<typeof schema>,
	masjidId: string,
	updatedPrayers: string[],
	updatedMasjid: {
		id: string;
		nameEn: string;
		nameHi: string;
	...
	try {
		...
		const validUsers = pinnedMasjids
			...
			.map(({ pushToken, preferredLanguage }) => ({
				pushToken,
				preferredLanguage: preferredLanguage as "en" | "hi" | "ur",
			}));

		if (validUsers.length === 0) return;

		// Create individual messages for each user with their preferred language
		const messages: ExpoPushMessage[] = validUsers.map(
			({ pushToken, preferredLanguage }) => {
				const { title, body } = generateLocalizedNotification(
					updatedMasjid,
					updatedPrayers,
					preferredLanguage,
				);
		...
		const invalidTokens = tickets
			...
			.map(
				(ticket) => ticket.message?.match(/"(ExponentPushToken\[.*?\])"/)?.[1],
			)
		...
		if (invalidTokens.length > 0) {
			try {
				...
				console.log(
					`Removed ${invalidTokens.length} invalid push tokens from database`,
				);
			} catch (deleteError) {
...
const masjidRoutes = new Hono<{ Bindings: Bindings; Variables: Variables }>()
	...
	.get("/", async (c) => {
		...
		if (!masjid) {
			return c.json(
				{ success: false, message: "Masjid profile not found." },
				404,
			);
		}
	...
	.post(
		...
		async (c) => {
			...
			if (existingMasjid) {
				return c.json(
					{ success: false, message: "A masjid profile already exists." },
					409,
				);
			}

			const { name: localizedName, address: localizedAddress } =
				await getMasjidTranslations(name, address, c.env);
			...
			return c.json(
				{ success: true, message: "Masjid profile created.", data: newMasjid },
				201,
			);
		},
	)
	...
	.put("/prayer-times", zValidator("json", prayerTimesSchema), async (c) => {
		...
		if (!updatedMasjid) {
			return c.json(
				{ success: false, message: "Masjid not found for this Imam." },
				404,
			);
		}
