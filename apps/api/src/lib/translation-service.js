async function translateText(text, targetLanguage, env) {
    ...
    try {
        ...
        console.warn(`API domain request failed for ${targetLanguage}, trying public endpoint:`, error);
    }
    ...
    try {
        ...
        try {
            ...
            console.warn(`Failed to parse public API response for ${targetLanguage}:`, e);
        }
    }
    catch (error) {
        console.warn(`Public API request also failed for ${targetLanguage}:`, error);
    }
    // If both methods fail, return the original text
    return text;
}
export async function getTranslations(text, env) {
    try {
...
export async function getMasjidTranslations(name, address, env) {
    try {
        ...
        const splitTranslation = (translatedText, originalName, originalAddress) => {
